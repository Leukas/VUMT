...
def load_paraphrase_data(params, data):
    """
    Load paraphrase data.
    """
    # assert len(params.paraphrase_dataset) > 0

    for lang, path in params.paraphrase_dataset.items():

        assert lang in params.langs
        logger.info('============ Paraphrase data (%s)' % (lang))
        ...
        paraphrase_data = ParallelDataset(
            ...
            data2['sentences'], data2['positions'], data['dico'][lang], params.lang2id[lang],
            params
        )
...
def check_all_data_params(params):
    ...
    # check paraphrase datasets
    params.paraphrase_dataset = {k: v for k, v in [x.split(':') for x in params.paraphrase_dataset.split(';') if len(x) > 0]}
...
def load_data(params, mono_only=False):
    ...
    """
    data = {'dico': {}, 'mono': {}, 'para': {}, 'back': {}, 'paraphrase': {}}
    ...
    # paraphrase datasets
    load_paraphrase_data(params, data)
